...
Мок имитирует поведение реальной симуляции: если в scenario['failures']
встречается конкретный "критичный" спутник (S05), у клиента C2
availability резко падает. Так можно проверить логику ранжирования
независимо от geometry.py/router.py/analyzer.py.
"""
...
def mock_simulate_fn(scenario: dict) -> SimulationResult:
    ...
    - если среди scenario['failures'] есть спутник 'S05' (отказавший на
      весь горизонт) -> у клиента C2 availability падает до 0.40;
